...
/**
 * Strict commit action (VSCode-style "Commit Staged").
 * Only commits changes that have been explicitly staged.
 * Refuses to commit if nothing is staged.
 */
public class CommitStagedAction extends AbstractAction {
    private static final boolean DEBUG = false;

    private GitPanel gitPanel;

    public CommitStagedAction(GitPanel gitPanel) {
        super("Commit Staged");
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        try {
            ...
            if (!hasStagedChanges) {
                JOptionPane.showMessageDialog(gitPanel,
                    "No staged changes to commit.\nStage files first using the context menu or Stage All button.",
            ...
            if (DEBUG) {
                System.out.println("CommitStagedAction: Committed: " + commit.getId().getName());
            }
