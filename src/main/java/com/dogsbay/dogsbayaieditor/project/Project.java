...
import com.dogsbay.dogsbayaieditor.project.actions.AddDirectoryAction;
import com.dogsbay.dogsbayaieditor.project.actions.AddDirectoryContentsAction;
import com.dogsbay.dogsbayaieditor.project.actions.AddFileAction;
import com.dogsbay.dogsbayaieditor.project.actions.AddFolderAction;
import com.dogsbay.dogsbayaieditor.project.actions.AddRemoteDocumentAction;
import com.dogsbay.dogsbayaieditor.project.actions.AddVirtualDirectoryAction;
import com.dogsbay.dogsbayaieditor.project.actions.DeleteAction;
import com.dogsbay.dogsbayaieditor.project.actions.DeleteProjectAction;
import com.dogsbay.dogsbayaieditor.project.actions.FindInProjectsAction;
import com.dogsbay.dogsbayaieditor.project.actions.ImportProjectAction;
import com.dogsbay.dogsbayaieditor.project.actions.NewProjectAction;
import com.dogsbay.dogsbayaieditor.project.actions.OpenFileAction;
import com.dogsbay.dogsbayaieditor.project.actions.ParseAction;
import com.dogsbay.dogsbayaieditor.project.actions.ProjectPropertiesAction;
import com.dogsbay.dogsbayaieditor.project.actions.RefreshVirtualDirectoryAction;
import com.dogsbay.dogsbayaieditor.project.actions.RemoveFileAction;
import com.dogsbay.dogsbayaieditor.project.actions.RemoveFolderAction;
import com.dogsbay.dogsbayaieditor.project.actions.RenameFolderAction;
import com.dogsbay.dogsbayaieditor.project.actions.RenameProjectAction;
import com.dogsbay.dogsbayaieditor.project.actions.ValidateAction;
...
public class Project extends JPanel implements TreeWillExpandListener  {
	 ...
	 */
	public Project( DogsBayAIEditor parent, ConfigurationProperties props) {
...
//		getNewProjectAction().setEnabled( enabled);
	...
	public void findInFiles( final String search, final boolean regExp, final boolean matchCase, final boolean wholeWord) {
		...
		if ( node instanceof FolderNode) {
			final DogsBayProgressDialog progressDialog  = new DogsBayProgressDialog(parent, true);
...
//						 reset previous find in files + select
	...
	public void importProject( File project, boolean isStartup) {
		try {
			if ( project.exists()) {
				URL url = DogsBayURLUtilities.getURLFromFile(project);
						...
						if ( name != null && name.trim().length() > 0) {
							...
							if ( existingProject != null) {
								...
								if ( value == JOptionPane.OK_OPTION){
									...
									// Add documents and folders BEFORE adding to properties
									// (addProjectProperties clones the XElement, so we need contents added first)
									...
									addVirtualFolders( base, project, virtualFolders);

									// Now add to properties (this will clone the XElement with all contents included)
	...
	private void addFolders( URL base, FolderProperties parent, XElement[] folders) {
		...
		for ( int i = 0; i < folders.length; i++) {
			...
			if ( name != null && name.trim().length() > 0) {
				...
				// Add documents and child folders BEFORE adding to parent
				// (addFolderProperties clones the XElement, so we need contents added first)
				...
				addVirtualFolders( base, folder, virtualFolders);

				// Now add to parent (this will clone the XElement with all contents included)
