...
public class ChangeDocumentDialog extends JDialog {
	...
	public ChangeDocumentDialog( JFrame parent) {
		...
		KeyStroke altOne = KeyStroke.getKeyStroke(KeyEvent.VK_1,InputEvent.ALT_DOWN_MASK, false);
	 ...
	 */
	public void show( Vector views, DogsBayView current, DogsBayView previous) 
	{
		// intialise the list
	...
	private void setListContent( Vector views, DogsBayView current, DogsBayView previous)
	{
	...
	private void addToModel( DogsBayView view)
	{
	...
	public DogsBayView getSelectedView() {
		int index = list.getSelectedIndex();
		if ( index >= 0) {
			return (DogsBayView)tabPanes.get( index);
		}
