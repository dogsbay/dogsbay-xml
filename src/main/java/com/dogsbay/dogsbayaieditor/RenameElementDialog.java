 ...
 * @author Dogsbay
 */
public class RenameElementDialog extends DogsBayDialog implements Editor.CurrentSiblingsListener {
	 ...
	 */
	public RenameElementDialog( JFrame parent) {
		super( parent, true);
		
		setResizable( false);
		setTitle( "Rename Element");
		setDialogDescription( "Specify a new Element Name");
		...
		namePanel.add( icon);
		namePanel.add( prefix);
		namePanel.add( colon);
		namePanel.add( name);
		
		namePanel.setBorder( new EmptyBorder( 0, 2, 5, 0));
...
//				System.out.println( "AncestorListener.ancestorAdded() ["+TagDialog.this.isVisible()+"]");
		...
		tagPanel.add( namePanel, FormLayout.RIGHT);
...
//		Vector unorderedNames = names;
	 ...
	 */
	public void init( Editor editor, Tag tag) {
		this.editor = editor;
		
		if ( tag != null) {
			setElementPrefix( tag.getPrefix());
			setElementName( tag.getName());
		} else {
			setElementPrefix( null);
			setElementName( null);
		}

		prefix.setFont( TextPreferences.getBaseFont().deriveFont( Font.PLAIN + Font.ITALIC));
		colon.setFont( TextPreferences.getBaseFont().deriveFont( Font.PLAIN));
		name.setFont( TextPreferences.getBaseFont().deriveFont( Font.BOLD));
//		icon.setPreferredSize( new Dimension( 16, colon.getPreferredSize().height));
	...
	private void updateNames() {
		if ( editor.getAllElements() == null || editor.getAllElements().size() == 0) {
			...
			editor.getCurrentSiblings( this);
		}
	}
	
	public void setCurrentSiblings( Vector lists) {
//		System.out.println( "RenameElementDialog.setCurrentSiblings( "+lists+")");
	...
	private void setElementPrefix( String value) {
	...
	private void setElementName( String value) {
		name.setText( value);
	}
