 ...
 * @author Dogsbay
 */
public class TemplateManagementDialog extends DogsBayDialogHeader {
	 ...
	 */
	public TemplateManagementDialog( DogsBayAIEditor parent, ConfigurationProperties props) {
		super( parent, true);
...
//		mainPanel.add( text, BorderLayout.NORTH);
	...
	/**
	 * Shows the dialog and initializes the template list.
	 * Updated for Java 21 compatibility - replaced deprecated show() with setVisible().
	 */
	@Override
	public void setVisible(boolean visible) {
		if (visible) {
			// Initialize template list before showing
...
//	  *******************START new code*********************************
	...
	private JFileChooser getImportChooser() {
		if ( importChooser == null) {
			...
			importChooser.addChoosableFileFilter( new DefaultFileFilter( "template templates", "DogsBay XML Template(s) Document"));
		} 
	...
	private JFileChooser getExportChooser() {
		if ( exportChooser == null) {
			...
			exportChooser.addChoosableFileFilter( new DefaultFileFilter( "template", "DogsBay XML Template Document"));
		} 
	...
	private void exportDocument( URL url, TemplateProperties props) throws IOException, SAXParseException {
		DogsBayDocument document = new DogsBayDocument( url, props.exportTemplate( url));
