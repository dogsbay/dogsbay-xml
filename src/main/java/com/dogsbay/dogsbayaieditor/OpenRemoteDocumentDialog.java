 ...
 * @author Dogsbay
 */
public class OpenRemoteDocumentDialog extends DogsBayDialog {
	...
	private static final String[] OPEN_PROTOCOLS = { PROTOCOL_HTTP, PROTOCOL_HTTPS, PROTOCOL_WEBDAV_HTTP, PROTOCOL_WEBDAV_HTTPS, PROTOCOL_FTP};
	 ...
	 */
	public OpenRemoteDocumentDialog( ConfigurationProperties properties, JFrame parent) {
		super( parent,true);
		
		this.properties = properties;
		
		setTitle( "Open Remote Document");
...
//		JLabel nameLabel = new JLabel( "Name:");
		...
		protocolField = new JComboBox( OPEN_PROTOCOLS);
...
//			System.out.println( (URL)urls.elementAt(i));
	...
	public URL getURL() {
		...
		if ( item == PROTOCOL_HTTPS || item == PROTOCOL_WEBDAV_HTTPS) {
			protocol = "https";
		...
		if ( string != null && string.trim().length() > 0) { 
			try {
				...
				if ( protocol.equals( "http")) {
					protocolField.setSelectedItem( PROTOCOL_HTTP);
				} else if ( protocol.equals( "https")) {
					protocolField.setSelectedItem( PROTOCOL_HTTPS);
