...
import com.dogsbay.dogsbayaieditor.OpenDocument;
 ...
 * @author Dogsbay
 */
public class ExecuteXSLTDialog extends DogsBayDialog {
	...
	private JPanel getInputPanel() {
		if (inputPanel == null) {
			...
			inputPromptButton.setPreferredSize(new Dimension(inputPromptButton.getPreferredSize().width,
					inputLocationField.getPreferredSize().height));
	...
	private JPanel getOutputPanel() {
		if (outputPanel == null) {
			...
			outputLocationButton.setPreferredSize(new Dimension(outputLocationButton.getPreferredSize().width,
					outputLocationField.getPreferredSize().height));
			...
			outputToFileButton.setPreferredSize(new Dimension(outputToFileButton.getPreferredSize().width,
					outputLocationField.getPreferredSize().height));
			...
			outputToNewDocumentButton.setPreferredSize(new Dimension(outputToNewDocumentButton.getPreferredSize().width,
					outputLocationField.getPreferredSize().height));
			outputPanel.add(outputToNewDocumentButton, FormLayout.FULL);

			outputToInputButton = new JRadioButton("To Input Document");
			outputToInputButton.setPreferredSize(new Dimension(outputToInputButton.getPreferredSize().width,
					outputLocationField.getPreferredSize().height));
			...
			outputToBrowserButton.setPreferredSize(new Dimension(outputToBrowserButton.getPreferredSize().width,
					outputLocationField.getPreferredSize().height));
	...
	private JPanel getXSLPanel() {
		if (xslPanel == null) {
			...
			xslLocationButton.setPreferredSize(new Dimension(xslLocationButton.getPreferredSize().width,
					xslLocationField.getPreferredSize().height));
			...
			xslFromURLButton.setPreferredSize(new Dimension(xslFromURLButton.getPreferredSize().width,
					xslLocationField.getPreferredSize().height));
			...
			xslCurrentButton.setPreferredSize(new Dimension(xslCurrentButton.getPreferredSize().width,
					xslLocationField.getPreferredSize().height));

			xslFromOpenDocumentButton = new JRadioButton("Open Document");
			xslFromOpenDocumentButton.setPreferredSize(new Dimension(xslFromOpenDocumentButton.getPreferredSize().width,
					inputLocationField.getPreferredSize().height));

			xslFromOpenDocumentBox = new JComboBox();
			// inputFromOpenDocumentBox.setFont(
			// inputFromOpenDocumentBox.getFont().deriveFont( Font.PLAIN));
			xslFromOpenDocumentBox.setPreferredSize(new Dimension(xslFromOpenDocumentBox.getPreferredSize().width,
					inputLocationField.getPreferredSize().height));
			xslFromOpenDocumentBox.setEditable(true);

			xslPromptButton = new JRadioButton("Prompt For Stylesheet");
			xslPromptButton.setPreferredSize(new Dimension(xslPromptButton.getPreferredSize().width,
					xslLocationField.getPreferredSize().height));

			xslPanel.add(xslPromptButton, FormLayout.FULL);

			xslFromPIsButton = new JRadioButton("Use Processing Instructions");
			xslFromPIsButton.setPreferredSize(new Dimension(xslFromPIsButton.getPreferredSize().width,
					xslLocationField.getPreferredSize().height));
	...
	private JDialog getProcessorDialog() {
		if (processorDialog == null) {
			processorDialog = new DogsBayDialogHeader(parent, true);
	...
	public ScenarioProperties getScenario() {
		...
		if (inputCurrentButton.isSelected()) {
			...
			properties.setInputFile(
					((OpenDocument) inputFromOpenDocumentBox.getSelectedItem()).getDocument().getURL().toString());
		...
		if (xslCurrentButton.isSelected()) {
			...
			properties.setXSLURL(
					((OpenDocument) xslFromOpenDocumentBox.getSelectedItem()).getDocument().getURL().toString());
	...
	private boolean setOpenDocuments() {
		...
		Vector views = ((DogsBayAIEditor) parent).getViews();
	...
	public void showDialog(DogsBayDocument document) {
		JPanel form = new JPanel(new FormLayout(10, 2));
	...
	private boolean isStylesheet(DogsBayDocument doc) {
		XElement root = doc.getRoot();

		return (root != null && root.getName().equals("stylesheet")
				&& root.getNamespaceURI().equals("http://www.w3.org/1999/XSL/Transform"));
	}
