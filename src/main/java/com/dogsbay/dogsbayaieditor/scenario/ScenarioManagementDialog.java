 ...
 * @author Dogsbay
 */
public class ScenarioManagementDialog extends DogsBayDialogHeader {
	private static final boolean DEBUG = false;
	private static final Dimension SIZE = new Dimension(300, 300 + DogsBayDialogHeader.HEADER_HEIGHT);
		...
		if (!getScenarioPropertiesDialog().isVisible()) {
			...
			if (scenario != null) {
				...
				if (parent instanceof DogsBayAIEditor) {
					document = ((DogsBayAIEditor) parent).getDocument();
				}
		...
		if (!getScenarioPropertiesDialog().isVisible()) {
			...
			if (selectedObjects.size() > 0) {
				...
				for (int cnt = 0; cnt < selectedObjects.size(); ++cnt) {
					...
					// if the deleteAll flag is false, then ask the user about each individual
					// object
					if (deleteAll == false) {
	...
	private void importScenario(URL url, XElement element) {
		if (DEBUG)
			System.out
					.println("scenarioManagementDialog.importType( " + url.toString() + ", " + element.getName() + ")");
		...
		if (name != null) {
			...
			for (int i = 0; i < scenarios.size(); i++) {
				...
				if (name.equals(name2)) {
					...
					int value = MessageHandler
							.showConfirm("A Scenario with this name \"" + name + "\" already Exists.\n" +
	...
	private File getSelectedChooserFile() {
		...
		if (parent instanceof DogsBayAIEditor) {
			doc = ((DogsBayAIEditor) parent).getDocument();
		}
	...
	private JFileChooser getImportChooser() {
		if (importChooser == null) {
			...
			importChooser.addChoosableFileFilter(
					new DefaultFileFilter("scenario scenarios", "DogsBay XML Scenario(s) Document"));
		}
	...
	private JFileChooser getExportChooser() {
		if (exportChooser == null) {
			...
			exportChooser.addChoosableFileFilter(new DefaultFileFilter("scenario", "DogsBay XML Scenario Document"));
		}
	...
	private void exportDocument(URL url, ScenarioProperties props) throws IOException, SAXParseException {
		DogsBayDocument document = new DogsBayDocument(url, props.exportScenario(url));
		...
		public ScenarioListModel(Vector list) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				for (int j = 0; j < elements.size() && index == -1; j++) {
					...
					if (element.getName()
							.compareToIgnoreCase(((ScenarioProperties) elements.elementAt(j)).getName()) <= 0) {
						index = j;
					}
				}
