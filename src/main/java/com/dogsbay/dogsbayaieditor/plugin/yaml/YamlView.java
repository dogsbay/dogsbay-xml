...
import com.dogsbay.xml.editor.FoldingPlainView;
import com.dogsbay.xml.editor.XmlContext;
import com.dogsbay.xml.editor.XmlDocument;

/**
 * View for rendering YAML with syntax highlighting.
 */
public class YamlView extends FoldingPlainView {
    private boolean highlight = false;
    private YamlScanner lexer;
    private boolean lexerValid;
    private XmlContext context;

    public YamlView(XmlContext context, Element elem) {
        super(elem);
        this.context = context;
        XmlDocument doc = (XmlDocument) getDocument();
        try { lexer = new YamlScanner(doc); } catch (Exception e) { lexer = null; }
        lexerValid = false;
    }

    public void paint(Graphics g, Shape a) { super.paint(g, a); lexerValid = false; }
    public void setHighlight(boolean enabled) { highlight = enabled; }

    protected int drawUnselectedText(Graphics g, int x, int y, int start, int end) throws BadLocationException {
        if (lexer == null) return super.drawUnselectedText(g, x, y, start, end);
        ...
        if (lastStyle == null) lastStyle = resolveStyle(null);
        Segment text = getLineBuffer();
    ...
    protected int drawSelectedText(Graphics g, int x, int y, int start, int end) throws BadLocationException {
        if (lexer == null) return super.drawSelectedText(g, x, y, start, end);
        Document doc = getDocument();
        int mark = start;
    ...
    private Style resolveStyle(Style style) {
        if (style == null) {
            Style def = context.getStyle(Constants.YAML_TEXT);
            return (def != null) ? def : context.getStyle(0);
        }
        return style;
    }
