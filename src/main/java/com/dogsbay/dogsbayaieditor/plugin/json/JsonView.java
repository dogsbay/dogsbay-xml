...
/**
 * View for rendering JSON with syntax highlighting.
 */
public class JsonView extends FoldingPlainView {
    private boolean highlight = false;
    private JsonScanner lexer;
    private boolean lexerValid;
    private XmlContext context;

    public JsonView(XmlContext context, Element elem) {
        super(elem);
        this.context = context;

        XmlDocument doc = (XmlDocument) getDocument();
        try {
            lexer = new JsonScanner(doc);
        } catch (Exception e) {
            lexer = null;
        }
        lexerValid = false;
    ...
    private Style resolveStyle(Style style) {
        if (style == null) {
            Style defaultStyle = context.getStyle(Constants.JSON_TEXT);
