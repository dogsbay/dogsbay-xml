...
//	private QButton tagCompletionField = null;
	 ...
	 */
	public PropertiesPanel( ConfigurationProperties props, DogsBayAIEditor editor) {
		...
		if (highlight == null) highlight = Color.white;
		this.setBorder(
				new CompoundBorder(
						new MatteBorder( 1, 1, 0, 0, highlight),
...
//				new BevelBorder( BevelBorder.RAISED));
		...
		new EtchedBorder( highlight, UIManager.getColor("controlDkShadow"))));
...
//		validationList.setModel( validationListModel);
		...
		basePanel.setBackground( panelBg);
...
//		closeButton.setIcon( getUpIcon());
	...
	private ImageIcon getUpIcon() { 
		if ( upIcon == null) {
			upIcon = DogsBayImageLoader.get().getImage( "com/dogsbay/dogsbayaieditor/icons/Up8.gif");
		}
	...
	private ImageIcon getDownIcon() { 
		if ( downIcon == null) {
			downIcon = DogsBayImageLoader.get().getImage( "com/dogsbay/dogsbayaieditor/icons/Down8.gif");
		}
