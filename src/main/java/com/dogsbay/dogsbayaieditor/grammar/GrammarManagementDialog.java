 ...
 * @author Dogsbay
 */
public class GrammarManagementDialog extends DogsBayDialogHeader {
...
//	private QLabel text = null;
	 ...
	 */
	public GrammarManagementDialog( DogsBayAIEditor parent, ConfigurationProperties props) {
		super( parent, true);
...
//		northButtonPanel.setBorder( new EmptyBorder( 2, 0, 5, 0));
	...
	/**
	 * Shows the dialog with current properties.
	 */
...
//	  *******************START new code*********************************
	    ...
	    Object[] selectedObjects = list.getSelectedValuesList().toArray();
	...
	private JFileChooser getImportChooser() {
		if ( importChooser == null) {
			...
			importChooser.addChoosableFileFilter( new DefaultFileFilter( "type types", "DogsBay XML Type(s) Document"));
		} 
	...
	private JFileChooser getExportChooser() {
		if ( exportChooser == null) {
			...
			exportChooser.addChoosableFileFilter( new DefaultFileFilter( "type", "DogsBay XML Type Document"));
		} 
	...
	private void exportDocument( URL url, GrammarProperties props) throws IOException, SAXParseException {
		DogsBayDocument document = new DogsBayDocument( url, props.exportType( url));
