 ...
 * @author Dogsbay
 */
public class GrammarPropertiesDialog extends DogsBayDialog {
	...
	private DogsBayDialogHeader namespaceDialog			= null;
...
//	private JButton baseLocationButton		= null;
	 ...
	 */
	public GrammarPropertiesDialog( DogsBayAIEditor parent, ConfigurationProperties props, String confirmText, boolean allowOverride) {
	 ...
	 * @param frame the parent frame.
	 */
	public GrammarPropertiesDialog( DogsBayAIEditor parent, ConfigurationProperties props, String confirmText) {
		super( parent, true);
...
//			System.out.println( "setGrammarIcon() [Setting default Icon...]");
			icon = DogsBayImageLoader.get().getImage( "com/dogsbay/dogsbayaieditor/icons/XMLDocumentIcon.gif");
		}
...
//	  *******************START new code*********************************
	    ...
	    //Object[] selectedObjects = scenarioList.getSelectedValuesList().toArray();
...
//	  *******************START new code*********************************
	...
	protected void okButtonPressed() {
		...
		if ( checkName( name)) {
			...
			// Validate that validation location is provided unless using document location
			if (!xmlValidationLocationCheck.isSelected()) {
				String validationLoc = validationLocationField.getText();
				if (validationLoc == null || validationLoc.trim().isEmpty()) {
					MessageHandler.showError(parent, "Please specify a Validation Location or enable 'Location defined in Document'.", "Validation Location Required");
					return;
				}
			}
...
//		nameField.setText( properties.getName());
	...
	private String getSchemaLocation( URL url, File file) {
		...
		try {
			result = com.dogsbay.xml.DogsBayURLUtilities.getURLFromFile(file);
			String schema = getFile( url);
			schema = stripExtension( schema);
			schema = schema+".xsd";
	...
	private JDialog getNamespaceDialog() {
		if ( namespaceDialog == null) {
			namespaceDialog = new DogsBayDialogHeader( parent, true);
...
//	  *******************START new code*********************************
	    ...
	    //Object[] selectedObjects = prefixNamespaceMappingList.getSelectedValuesList().toArray();
...
//	  *******************START new code*********************************
	    ...
	    //get the selected objects
	    Object[] selectedObjects = fragmentList.getSelectedValuesList().toArray();
...
//	  *******************START new code*********************************
	    ...
	    //get the selected objects
	    Object[] selectedObjects = tagCompletionList.getSelectedValuesList().toArray();
...
//	}
	...
	class FragmentListCellRenderer extends JPanel implements ListCellRenderer {
		...
		return configurationProperties.getKeyBindings().isTaken( stroke);
	}
