...
//	}
	 ...
	 *        for the editor.
	 */
	public GrammarProperties( ConfigurationProperties props, DogsBayDocument document) {
		super( new XElement( GRAMMAR_PROPERTIES));
		...
		if ( document != null) {
			...
			if ( !document.isError()) {
				...
				if ( docType != null) {
					...
					// Don't set validation location automatically - user must provide explicit location
					// Most public grammars (like DITA) have relative SYSTEM IDs that won't work
					// setValidationLocation( docType.getSystemID());

					// Don't automatically add tag completion for SYSTEM IDs that may be relative paths
					// Users can manually configure tag completion or use "Use for Tag Completion" option
...
//					setTemplateGrammar( XMLGrammar.TYPE_DTD);
					...
					if ( location != null) {
						...
						// Don't set validation location automatically - user must provide explicit location
						// Schema locations may be relative paths that won't work
						// setValidationLocation( targetURI);

						// Don't automatically add tag completion for schema locations that may be relative paths
						// Users can manually configure tag completion or use "Use for Tag Completion" option
	...
	public XElement exportType( URL url) { // throws IOException, SAXParseException {
		//String namespace = "http://www.dogsbay.ai/dogsbay-editor/"+Identity.getIdentity().getVersion()+"/";
	...
	public boolean useXMLValidationLocation() {
		return getBoolean( USE_XML_VALIDATION_LOCATION, false);
	}
...
// Implementation of XMLGrammar
	...
	private static XElement addElement( XElement element, String name, String value) {
		//XElement e = new XElement( name, "http://www.dogsbay.ai/dogsbay-editor/"+Identity.getIdentity().getVersion()+"/");
