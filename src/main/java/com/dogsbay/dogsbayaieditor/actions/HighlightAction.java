 ...
 *
 * @version	$Revision: 1.2 $, $Date: 2004/07/15 15:28:54 $
 * @author Dogsbay
 */
 public class HighlightAction extends AbstractAction {
	 ...
	 */
 	public HighlightAction( DogsBayAIEditor parent) {
		super( "Highlight");
		
		this.parent = parent;

		if (DEBUG) System.out.println( "HighlightAction( "+editor+")");
		
//		putValue( ACCELERATOR_KEY, KeyStroke.getKeyStroke( KeyEvent.VK_DOWN, InputEvent.CTRL_DOWN_MASK, false));
		putValue( SMALL_ICON, DogsBayImageLoader.get().getImage( "com/dogsbay/dogsbayaieditor/icons/Highlight16.gif"));
		putValue( SHORT_DESCRIPTION, "Highlight");
	...
	public void setDocument( DogsBayDocument doc) {
		if ( doc != null && (doc.isXML() || doc.isMarkdown())) {
	 ...
	 */
 	public void actionPerformed( ActionEvent event) {
 		if (DEBUG) System.out.println( "HighlightAction.actionPerformed( "+event+")");
		
		editor.setHighlight( !editor.isHighlight());
		editor.setFocus();
 	}
}
