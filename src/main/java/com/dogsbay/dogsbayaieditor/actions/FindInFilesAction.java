...
import com.dogsbay.dogsbayaieditor.FindInFilesDialog;
...
public class FindInFilesAction extends AbstractAction {
	...
	private FindInFilesDialog findDialog = null;
 	...
 	DogsBayProgressDialog progressDialog = null;
	 ...
	 */
 	public FindInFilesAction( DogsBayAIEditor parent, ConfigurationProperties props) {
 		super( "Find in Files ...");
...
//		putValue( SMALL_ICON, DogsBayImageLoader.get().getImage( "com/dogsbay/dogsbayaieditor/icons/Copy16.gif"));
	...
	/**
	 * Opens the search panel in search-only mode.
	 *
	 * @param e the action event.
	 */
 	public void actionPerformed( ActionEvent e) {
	 	// Use the new VSCode-style search panel instead of dialog
	 	if (parent.getSearchPanel() != null) {
	 		parent.getSearchPanel().activateSearchMode();
	 	}

	 	/* OLD DIALOG-BASED IMPLEMENTATION - Replaced with search panel
	 	if ( findDialog == null) {
	 	...
	 	if ( !findDialog.isCancelled()) {
	 		...
	 		if ( search != null) {
				...
				//project.findInFiles( search, regExp, matchCase, wholeWord);
	 			progressDialog  = new DogsBayProgressDialog(parent, true);
								...
								for ( int i = 0; i < documents.size(); i++) {
									try {
										...
										try {
											...
											newURL = com.dogsbay.xml.DogsBayURLUtilities.getURLFromFile(doc);
											...
											try {
												origURL = com.dogsbay.xml.DogsBayURLUtilities.getURLFromFile(doc);
