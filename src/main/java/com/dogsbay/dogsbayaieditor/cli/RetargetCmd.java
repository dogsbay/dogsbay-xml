...
@Command(name = "retarget",
        description = "Rewrite every reference from one target file to another "
                + "(no move, no delete). Dry-run by default — pass --apply to execute.")
class RetargetCmd implements Callable<Integer> {

    @Parameters(index = "0", description = "The currently referenced file")
    private String from;

    @Parameters(index = "1", description = "The new target file")
    private String to;
    ...
    public Integer call() throws Exception {
        ...
        RefactorResult result = executor.execute(new RetargetCommand(from, to, root, apply));
        return RefactorCliOutput.print(result,
                (result.applied() ? "Retargeted " : "Would retarget ")
