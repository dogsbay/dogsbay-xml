...
@Command(name = "rename-key",
        description = "Rename a key: all keydef definitions and every keyref/conkeyref. "
                + "Dry-run by default — pass --apply to execute.")
class RenameKeyCmd implements Callable<Integer> {

    @Parameters(index = "0", description = "The key to rename")
    private String oldKey;

    @Parameters(index = "1", description = "The new key name")
    private String newKey;

    @Option(names = {"-r", "--root"}, required = true,
            description = "Project root to scan and rewrite")
    private String root;
    ...
    public Integer call() throws Exception {
        ...
        RefactorResult result = executor.execute(
                new RenameKeyCommand(oldKey, newKey, root, apply));

        for (String warning : result.warnings()) {
            System.out.println("WARNING: " + warning);
        }
        System.out.println((result.applied() ? "Renamed key " : "Would rename key ")
