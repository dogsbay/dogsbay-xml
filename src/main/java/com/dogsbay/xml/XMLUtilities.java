...
public class XMLUtilities {
	...
	/**
	 * Returns a new XML reader with grammar type support.
	 * Does not use cached readers to ensure proper entity resolver configuration.
	 *
	 * @param validate true when the XML reader should validate.
	 * @param properties configuration properties for grammar type resolution
	 *
	 * @return the reader.
	 */
	public static SAXReader getReader( boolean validate,
			com.dogsbay.dogsbayaieditor.properties.ConfigurationProperties properties) {
		// Don't cache readers when using grammar type resolver
		// to ensure correct entity resolver is always used
		if ( validate) {
			return createReader( true, false, properties);
		} else if ( loadDTDGrammar) {
			return createReader( false, true, properties);
		} else {
			return createReader( false, false, properties);
		}
	}
	...
	public static SAXReader createReader( boolean validate, boolean loadExternalDTD) {
		return createReader( validate, loadExternalDTD, null);
	}

	/**
	 * Creates a new SAXReader with grammar type support.
	 *
	 * @param validate when true the reader validates the input.
	 * @param loadExternalDTD when true loads external DTD
	 * @param properties configuration properties for grammar type resolution (may be null)
	 *
	 * @return the reader.
	 */
	public static SAXReader createReader( boolean validate, boolean loadExternalDTD,
		...
		if ( !validate) {
			...
			try {
				// Block external general entities (the classic file-read XXE vector) in
				// both modes; DITA/XHTML content entities are internal to their DTD module
				// so this is safe. Parameter entities stay enabled when a DTD is loaded so
				// those modules resolve via the grammar/catalog resolver below.
				reader.setFeature( "http://xml.org/sax/features/external-general-entities", false);

				if ( loadExternalDTD) {
					reader.setFeature( "http://apache.org/xml/features/nonvalidating/load-dtd-grammar", true);
					// Use grammar type resolver if properties available, otherwise catalog only
					if ( properties != null) {
						reader.setEntityResolver( new GrammarTypeEntityResolver(properties));
					} else {
					...
					// No grammar resolver in this mode — block internal-subset external ids too.
	...
	public static SAXReader createReader( boolean validate, boolean loadExternalDTD, boolean stripWhiteSpace) {
		return createReader( validate, loadExternalDTD, stripWhiteSpace, null);
	}
	 ...
	 * @param validate when true the reader validates the input.
	 * @param loadExternalDTD when true loads external DTD
	 * @param stripWhiteSpace when true strips whitespace
	 * @param properties configuration properties for grammar type resolution (may be null)
	 *
	 * @return the reader.
	 */
...
//	}
	...
	public static synchronized XDocument parse( BufferedReader reader, int length, String systemId, String grammarLocation, boolean stripWhiteSpace) throws IOException, SAXParseException{
		return parse( reader, length, systemId, grammarLocation, stripWhiteSpace, null);
	}

	/**
	 * Parses the document for this reader with grammar type support.
	 *
	 * @param reader the reader with all the information.
	 * @param systemId the systemId of the document.
	 * @param grammarLocation the location of the grammar.
	 * @param stripWhiteSpace whether to strip whitespace.
	 * @param properties the configuration properties for grammar type resolution.
	 *
	 * @return the Dom4J document.
	 */
	public static synchronized XDocument parse( BufferedReader reader, int length, String systemId, String grammarLocation, boolean stripWhiteSpace, com.dogsbay.dogsbayaieditor.properties.ConfigurationProperties properties) throws IOException, SAXParseException{
		if (DEBUG) System.out.println( "XMLUtilities.parse( "+reader+", "+systemId+", "+grammarLocation+")");


		SAXReader saxReader = createReader( false, loadDTDGrammar, stripWhiteSpace, properties);
...
//			// parse without substituting the entities
	...
	public static synchronized XDocument parse( BufferedReader reader, int length, String systemId, String grammarLocation) throws IOException, SAXParseException{
		return parse( reader, length, systemId, grammarLocation, false, null);
	}
	 ...
	 * @return the Dom4J document.
	 */
	public static synchronized XDocument parse( BufferedReader reader, int length, String systemId, String grammarLocation, com.dogsbay.dogsbayaieditor.properties.ConfigurationProperties properties) throws IOException, SAXParseException{
		return parse( reader, length, systemId, grammarLocation, false, properties);
	}
	...
	public static synchronized XDocument parse( BufferedReader reader, int length, String systemId) throws IOException, SAXParseException {
		return parse( reader, length, systemId, (com.dogsbay.dogsbayaieditor.properties.ConfigurationProperties)null);
	}
	 ...
	 * @return the Dom4J document.
	 */
	public static synchronized XDocument parse( BufferedReader reader, int length, String systemId, com.dogsbay.dogsbayaieditor.properties.ConfigurationProperties properties) throws IOException, SAXParseException {
		if (DEBUG) System.out.println( "XMLUtilities.parse( "+reader+", "+systemId+")");

		SAXReader saxReader = createReader( false, loadDTDGrammar, properties);
...
//			 parse without substituting the entities
	...
	public static synchronized XDocument parse( BufferedReader reader, int length, String systemId, boolean stripWhiteSpace) throws IOException, SAXParseException {
		return parse( reader, length, systemId, stripWhiteSpace, null);
	}
	 ...
	 * @return the Dom4J document.
	 */
	public static synchronized XDocument parse( BufferedReader reader, int length, String systemId, boolean stripWhiteSpace, com.dogsbay.dogsbayaieditor.properties.ConfigurationProperties properties) throws IOException, SAXParseException {
		if (DEBUG) System.out.println( "XMLUtilities.parse( "+reader+", "+systemId+")");
...
//				x.printStackTrace();
	...
	public static String resolve( String publicId, String systemId) {
		...
		if ( entity != null && entity.startsWith( "file:") && !entity.startsWith( "file:/")) {
			...
			try {
				url = DogsBayURLUtilities.getURLFromFile(new File( entity));
				entity = url.toString();
			} catch ( MalformedURLException e) {
				e.printStackTrace();
			}
...
//			System.out.println( "XMLUtilities.resolve( "+publicId+", "+systemId+") ["+systemId+"]");
	...
	/**
	 * Resolves a PUBLIC/SYSTEM identifier using catalogs first, then grammar types.
	 * This method provides fallback resolution for documents when catalogs are not configured.
	 *
	 * @param publicId the PUBLIC identifier
	 * @param systemId the SYSTEM identifier
	 * @param properties configuration properties containing grammar type definitions
	 * @return resolved entity location, or systemId if not resolved
	 */
	public static String resolveWithGrammarTypes( String publicId, String systemId,
			com.dogsbay.dogsbayaieditor.properties.ConfigurationProperties properties) {
		// First try catalog resolution (existing behavior)
		String resolved = resolve( publicId, systemId);

		// If catalog didn't resolve it (returned original systemId), try grammar types
		if ( resolved != null && resolved.equals( systemId) && properties != null) {
			java.util.Vector grammarProps = properties.getGrammarProperties();

			for ( int i = 0; i < grammarProps.size(); i++) {
				com.dogsbay.dogsbayaieditor.grammar.GrammarProperties grammar =
					(com.dogsbay.dogsbayaieditor.grammar.GrammarProperties) grammarProps.elementAt(i);

				// Match by PUBLIC ID (preferred for DTDs like DITA)
				if ( publicId != null && !publicId.isEmpty()) {
					String grammarPublicId = grammar.getPublicID();
					if ( grammarPublicId != null && grammarPublicId.equals( publicId)) {
						// Found matching grammar type
						if ( !grammar.useXMLValidationLocation()) {
							String validationLocation = grammar.getValidationLocation();
							if ( validationLocation != null && !validationLocation.isEmpty()) {
								return validationLocation;
							}
						}
						// If using document location, fall through to return systemId
						break;
					}
				}

				// Fallback: match by SYSTEM ID if no PUBLIC ID match
				if ( systemId != null && !systemId.isEmpty()) {
					String grammarSystemId = grammar.getSystemID();
					if ( grammarSystemId != null && grammarSystemId.equals( systemId)) {
						if ( !grammar.useXMLValidationLocation()) {
							String validationLocation = grammar.getValidationLocation();
							if ( validationLocation != null && !validationLocation.isEmpty()) {
								return validationLocation;
...
//    		e.printStackTrace();
	...
	public static synchronized void write( XDocument document, URL url) throws IOException {
		...
		try {
			// Use toURI() for proper cross-platform path handling
			documentFile = new File(url.toURI());
		} catch (URISyntaxException e) {
			// Fall back to getFile() if URI syntax is invalid
		...
		File parentDir = documentFile.getParentFile();
		...
		if (documentFile.exists() && !documentFile.canWrite()) {
			throw new IOException("Cannot write to file (read-only): " + documentFile.getAbsolutePath());
		}
		if (parentDir != null && !parentDir.canWrite()) {
			throw new IOException("Cannot write to directory (read-only): " + parentDir.getAbsolutePath());
		}
	 ...
	 */
	public static synchronized void write( XDocument document, URL url, DogsBayOutputFormat format) throws IOException {
		...
		FileOutputStream out = new FileOutputStream(documentFile);
		try {
			DogsBayXMLWriter writer = new DogsBayXMLWriter( out, format);
			writer.write( document);
			writer.flush();
			out.flush();
		} finally {
	...
	public static synchronized String write( XDocument document, DogsBayOutputFormat format) throws IOException {
...
//		XMLWriter writer = new XMLWriter( out, format);
	...
	public static synchronized String write( XDocument document, DogsBayOutputFormat format, boolean escapeEntities) throws IOException {
...
//				e.printStackTrace();
	...
	/**
	 * Entity resolver that uses both catalogs and grammar types for resolution.
	 * Provides fallback resolution using configured grammar types when catalogs don't resolve.
	 */
	public static class GrammarTypeEntityResolver implements EntityResolver {
		private com.dogsbay.dogsbayaieditor.properties.ConfigurationProperties properties;

		public GrammarTypeEntityResolver(com.dogsbay.dogsbayaieditor.properties.ConfigurationProperties properties) {
			this.properties = properties;
		}

		public InputSource resolveEntity( String publicId, String systemId) throws SAXException, IOException {
			try {
				// Use enhanced resolution with grammar types
				String resolved = resolveWithGrammarTypes( publicId, systemId, properties);

				URL url = URLUtilities.toURL( resolved);
				InputSource source = new InputSource( resolved);
				source.setByteStream( url.openStream());

				return source;
			} catch ( Exception e) {
				// Return empty source to avoid parsing errors
				return new InputSource( new StringReader( ""));
			}
		}
	}
...
//			}
	 ...
	 */
	public static String resolveXIncludes(String text,String encoding,URL url, DogsBayOutputFormat format)
		throws DocumentException,IOException
	{
 	...
 	public static String format( String text, String systemId, String encoding,
			...
			boolean trim, boolean preserveMixed, DogsBayOutputFormat format) throws IOException, SAXParseException {
		return format( text, systemId, encoding, indent, newLines, padText, lineLength,
				trim, preserveMixed, format, java.util.Set.of());
	}

	/**
	 * As the 10-arg {@code format}, but {@code preserveSpaceElements} names elements
	 * (e.g. {@code codeblock}) whose whitespace is significant — they are emitted
	 * verbatim, never indented or trimmed.
	 */
 	public static String format( String text, String systemId, String encoding,
 			String indent, boolean newLines, boolean padText, int lineLength,
			boolean trim, boolean preserveMixed, DogsBayOutputFormat format,
			java.util.Set<String> preserveSpaceElements) throws IOException, SAXParseException {
		return format( text, systemId, encoding, indent, newLines, padText, lineLength,
				trim, preserveMixed, format, preserveSpaceElements, false);
	}

	/**
	 * As the 11-arg {@code format}, but when {@code preserveTextLineBreaks} is true,
	 * author line breaks inside prose (semantic line breaks / one-sentence-per-line)
	 * are preserved rather than collapsed; structural indentation is still normalized.
	 */
 	public static String format( String text, String systemId, String encoding,
 			String indent, boolean newLines, boolean padText, int lineLength,
			boolean trim, boolean preserveMixed, DogsBayOutputFormat format,
			java.util.Set<String> preserveSpaceElements,
			boolean preserveTextLineBreaks) throws IOException, SAXParseException {
		return format( text, systemId, encoding, indent, newLines, padText, lineLength,
				trim, preserveMixed, format, preserveSpaceElements, preserveTextLineBreaks, false);
	}

	/**
	 * As the 12-arg {@code format}, but when {@code preserveBlankLines} is true a single
	 * blank line between block siblings is preserved (runs of blanks collapse to one).
	 */
 	public static String format( String text, String systemId, String encoding,
 			String indent, boolean newLines, boolean padText, int lineLength,
			boolean trim, boolean preserveMixed, DogsBayOutputFormat format,
			java.util.Set<String> preserveSpaceElements,
			boolean preserveTextLineBreaks, boolean preserveBlankLines) throws IOException, SAXParseException {
...
//		format.setEncoding( doc.getEncoding());
		...
		formatter.setPreserveSpaceElements( preserveSpaceElements);
		formatter.setPreserveTextLineBreaks( preserveTextLineBreaks);
		formatter.setPreserveBlankLines( preserveBlankLines);
		...
		DogsBayOutputFormat format2 = new DogsBayOutputFormat( "", false, encoding);
		...
		// DogsBayXMLWriter's max line length is a global static (set from the editor's
		// Wrap Text preference at startup). Override it with THIS format's line length so
		// the second pass honors the same no-wrap (or wrap-column) contract as the first,
		// independent of the user's editor preference; restore it afterwards.
		int previousMaxLineLength = DogsBayXMLWriter.getMaxLineLength();
		DogsBayXMLWriter.setMaxLineLength( lineLength);
		try {
			DogsBayXMLWriter writer = new DogsBayXMLWriter( out, format2);
			writer.write( doc);
			writer.flush();

			return out.toString( mapXMLEncodingToJava( encoding));
		} finally {
			DogsBayXMLWriter.setMaxLineLength( previousMaxLineLength);
		}
 	}

	/**
	 * Reflow prose to one sentence per line (semantic line breaks) and canonically
	 * format the result. DOM-aware: only text inside ordinary flow content is
	 * reflowed — verbatim elements (codeblock/pre/lines/screen and any
	 * {@code xml:space="preserve"} subtree) are left untouched. A deliberate,
	 * writer-invoked operation (see {@link com.dogsbay.xml.format.SentenceReflow}).
	 */
	public static String reflowSentences( String text, String systemId, String encoding,
			com.dogsbay.xml.format.FormatStyle style) throws IOException, SAXParseException {
		ByteArrayInputStream stream = new ByteArrayInputStream( text.getBytes( mapXMLEncodingToJava( encoding)));
		InputStreamReader reader = new InputStreamReader( stream, mapXMLEncodingToJava( encoding));

		SAXReader sax = createReader( false, false);
		sax.setStripWhitespaceText( false);
		// Merge adjacent text so a prose run (and the whitespace around inline elements)
		// is one node — sentence boundaries that fall at a chunk split aren't missed,
		// and reflow stays idempotent.
		sax.setMergeAdjacentText( true);

		XDocument doc = XMLUtilities.parse( sax, new BufferedReader( reader), systemId);

		reflowElement( doc.getRootElement(), style.preserveSpaceElements(), false, 0, style);

		// Re-serialize the modified tree, then run the canonical formatter. Force
		// preserve-text-breaks ON for this pass: reflow's whole purpose is to insert
		// one-sentence-per-line breaks, and they must survive even when the project's
		// house style has preserve-text-breaks="false" (otherwise reflow un-does itself
		// for text-only paragraphs).
		return com.dogsbay.xml.format.FormatEngine.format(
				doc.asXML(), systemId, encoding, style.withPreserveTextLineBreaks( true));
	}

	/**
	 * Recursively reflow prose text nodes to one sentence per line, skipping
	 * whitespace-significant subtrees and whitespace-only nodes. Continuation lines
	 * are indented one level deeper than the containing element so the sentences
	 * align under the block (matching what the formatter then emits).
	 */
	private static void reflowElement( org.dom4j.Element element,
			java.util.Set<String> preserveSpace, boolean inPreserve, int depth,
			com.dogsbay.xml.format.FormatStyle style) {
		if ( element == null) {
			return;
		}
		boolean preserveHere = inPreserve
				|| preserveSpace.contains( element.getName())
				|| isXmlSpacePreserve( element);
		// "mixed" = this element directly contains prose text (so its inline children and
		// the whitespace around them are part of a sentence, not structure).
		boolean mixed = !preserveHere && hasProseText( element);
		// Continuation lines either align with the block's own indent (BLOCK, default) or
		// sit at column 0 (FLUSH) per the house style.
		String continuation = style.continuationIndent( depth);
		for ( int i = 0, size = element.nodeCount(); i < size; i++ ) {
			org.dom4j.Node node = element.node( i);
			if ( node instanceof org.dom4j.Element child) {
				reflowElement( child, preserveSpace, preserveHere, depth + 1, style);
			} else if ( !preserveHere && node instanceof org.dom4j.Text textNode) {
				String text = textNode.getText();
				if ( text.trim().length() > 0) {
					String reflowed = com.dogsbay.xml.format.SentenceReflow.reflow( text, continuation);
					if ( i == 0 && reflowed.startsWith( " ")) {
						reflowed = reflowed.substring( 1);   // no leading space after the open tag
					}
					textNode.setText( reflowed);
				} else if ( mixed) {
					// whitespace-only spacer between prose and an inline element: collapse to
					// a single space so the inline element stays on its sentence's line.
					textNode.setText( " ");
				}
				// else: element-only structural whitespace → leave for the formatter
			}
		}
	}

	private static boolean hasProseText( org.dom4j.Element element) {
		for ( int i = 0, size = element.nodeCount(); i < size; i++ ) {
			org.dom4j.Node n = element.node( i);
			if ( n instanceof org.dom4j.Text t && t.getText().trim().length() > 0) {
				return true;
			}
		}
		return false;
	...
	private static boolean isXmlSpacePreserve( org.dom4j.Element element) {
		org.dom4j.Attribute space = element.attribute( "space");
		return space != null && "preserve".equals( space.getValue())
				&& "xml".equals( space.getNamespacePrefix());
	}

	/**
	 * Collapse insignificant whitespace in prose/mixed content — runs of spaces/tabs
	 * become a single space, and whitespace at line ends and at block edges (right after
	 * an open tag / before a close tag) is removed. Verbatim subtrees
	 * (codeblock/pre/{@code xml:space="preserve"}) and the structural whitespace between
	 * block elements are left untouched. When the style preserves text line breaks,
	 * newlines and each line's leading indent (semantic-line-break continuation) are kept;
	 * otherwise prose collapses to a single flowing run. Returns the re-serialized tree
	 * (the caller formats it). Idempotent.
	 */
	public static String normalizeWhitespace( String text, String systemId, String encoding,
			com.dogsbay.xml.format.FormatStyle style) throws IOException, SAXParseException {
		ByteArrayInputStream stream = new ByteArrayInputStream( text.getBytes( mapXMLEncodingToJava( encoding)));
		InputStreamReader reader = new InputStreamReader( stream, mapXMLEncodingToJava( encoding));
		...
		XDocument doc = XMLUtilities.parse( sax, new BufferedReader( reader), systemId);
		normalizeElement( doc.getRootElement(), style.preserveSpaceElements(), false,
				style.preserveTextLineBreaks());
		return doc.asXML();
	}

	private static void normalizeElement( org.dom4j.Element element,
			java.util.Set<String> preserveSpace, boolean inPreserve, boolean keepNewlines) {
		if ( element == null) {
			return;
		}
		boolean preserveHere = inPreserve
		...
		for ( int i = 0, size = element.nodeCount(); i < size; i++ ) {
			org.dom4j.Node node = element.node( i);
			if ( node instanceof org.dom4j.Element child) {
				normalizeElement( child, preserveSpace, preserveHere, keepNewlines);
			} else if ( !preserveHere && node instanceof org.dom4j.Text textNode) {
				String text = textNode.getText();
				if ( text.trim().length() > 0) {
					String c = collapseHorizontal( text, keepNewlines);
					if ( i == 0) {
						c = stripLeadingWhitespace( c);   // insignificant right after the open tag
					}
					if ( i == size - 1) {
						c = stripTrailingWhitespace( c);   // insignificant right before the close tag
					}
					textNode.setText( c);
				} else if ( mixed) {
					// whitespace-only spacer: a single space between inline siblings, but
					// nothing at a block edge (right after open / before close).
					textNode.setText( ( i == 0 || i == size - 1) ? "" : " ");
				}
				// else: structural whitespace between block elements → formatter regenerates it
			}
		}
	}

	/** Collapse runs of spaces/tabs to one. When {@code keepNewlines}, newlines and each
	 *  line's leading indent are preserved and only line-end runs are trimmed. */
	private static String collapseHorizontal( String text, boolean keepNewlines) {
		if ( !keepNewlines) {
		...
		for ( int i = 0; i < lines.length; i++ ) {
			...
			if ( i == 0) {
				// First segment continues from the previous sibling (or the open tag), so its
				// leading whitespace is an inline boundary, not a line indent — collapse it too.
				String rest = line.replaceAll( "[ \t]+", " ");
				out.append( lastLine ? rest : rstripSpacesTabs( rest));
			} else {
				// Continuation line: preserve the leading indent (semantic line break), collapse
				// the rest, and strip the line-end run on non-final lines.
				int j = 0;
				while ( j < line.length() && ( line.charAt( j) == ' ' || line.charAt( j) == '\t')) {
					j++;
				}
				String rest = line.substring( j).replaceAll( "[ \t]+", " ");
				out.append( line, 0, j).append( lastLine ? rest : rstripSpacesTabs( rest));
			}
		}
		return out.toString();
	}

	private static String rstripSpacesTabs( String s) {
		int end = s.length();
	 ...
	 */
 	public static String updateXMLDeclaration(DogsBayDocument document,String version,String encoding,String standalone)
	{
	 ...
	 * 
	 */
 	public static void setXMLDoctype(DogsBayDocument document,String name,String type,String publicID,String systemID)
	{
	 ...
	 * 
	 */
 	public static void setSchemaLocation(DogsBayDocument document,String schemaType,String namespace,String schemaURL)
	{
