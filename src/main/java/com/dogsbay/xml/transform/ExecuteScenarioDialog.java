...
import com.dogsbay.dogsbayaieditor.StatusFrame;
...
//    private DogsBayDialogHeader header;
	...
	public ExecuteScenarioDialog( ConfigurationProperties props) {
		...
		setIconImage( (DogsBayImageLoader.get().getImage( "com/dogsbay/dogsbayaieditor/icons/ExecuteScenario16.gif")).getImage());
...
//		System.out.println( "setStatus( "+status+")");
	...
	public void execute( final ScenarioProperties scenario, final DogsBayDocument document) {
		if ( !busy) {
		...
		public Object getValueAt( int row, int column) {
			...
			if ( column == 0) {
				if ( status[row] == STATUS_ERROR) {
					result = DogsBayImageLoader.get().getImage( "com/dogsbay/xml/transform/icons/Error16.gif");
				} else if ( status[row] == STATUS_PROCESSED) {
					result = DogsBayImageLoader.get().getImage( "com/dogsbay/xml/transform/icons/Processed16.gif");
				} else if ( status[row] == STATUS_PROCESSING) {
					result = DogsBayImageLoader.get().getImage( "com/dogsbay/xml/transform/icons/Processing16.gif");
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
	   		...
	   		if ( model instanceof ScenarioTableModel) {
	   			...
	   			if ( rowType == ((ScenarioTableModel)model).ROW_TYPE_INPUT) {
					label.setIcon( DogsBayImageLoader.get().getImage( "com/dogsbay/xml/transform/icons/Input16.gif"));
	   			} else if ( rowType == ((ScenarioTableModel)model).ROW_TYPE_TRANSFORM) {
					label.setIcon( DogsBayImageLoader.get().getImage( "com/dogsbay/xml/transform/icons/Transform16.gif"));
	   			} else if ( rowType == ((ScenarioTableModel)model).ROW_TYPE_OUTPUT) {
					label.setIcon( DogsBayImageLoader.get().getImage( "com/dogsbay/xml/transform/icons/Output16.gif"));
	   			}
...
//	    header = new DogsBayDialogHeader(parent, true);
