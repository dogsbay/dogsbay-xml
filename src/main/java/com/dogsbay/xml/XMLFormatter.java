...
public class XMLFormatter extends XMLWriter {
    ...
    private DogsBayOutputFormat format = null;
    ...
    /** Element local names whose whitespace is significant (e.g. codeblock, pre) —
     *  emitted verbatim like {@code xml:space="preserve"}, never indented or trimmed. */
    private java.util.Set<String> preserveSpaceElements = java.util.Set.of();

    /** When true, author line breaks inside prose (semantic line breaks /
     *  one-sentence-per-line) are preserved rather than collapsed. */
    private boolean preserveTextLineBreaks = false;

    /** When true, a single blank line between block siblings is preserved (runs of
     *  blank lines collapse to one); when false, all blank lines are stripped. */
...
//    }
    ...
    public XMLFormatter( OutputStream out, DogsBayOutputFormat format) throws UnsupportedEncodingException {
        super( out, format);
    ...
    public XMLFormatter( DogsBayOutputFormat format) throws UnsupportedEncodingException {
        super( format);
    ...
    /** Set the element local names to emit verbatim (significant whitespace). */
    public void setPreserveSpaceElements( java.util.Set<String> names) {
        preserveSpaceElements = (names == null) ? java.util.Set.of() : names;
    }

    /** Preserve author line breaks inside prose (one-sentence-per-line) instead of
     *  collapsing them. Structural indentation is still normalized. */
    public void setPreserveTextLineBreaks( boolean preserve) {
        preserveTextLineBreaks = preserve;
    }

    /** Preserve a single blank line between block siblings (collapse multiple). */
    public void setPreserveBlankLines( boolean preserve) {
        preserveBlankLines = preserve;
    }

    /** True when {@code element} is directly preceded by a blank-line whitespace node
     *  that itself follows another element — i.e. a blank line between two blocks. */
    private static boolean precededByBlankLine( Element element) {
        Element parent = element.getParent();
        if ( parent == null) {
            return false;
        }
        java.util.List<Node> content = parent.content();
        int idx = -1;
        ...
        if ( idx < 2) {
            return false;   // need a whitespace node and an element before it
        }
        Node prev = content.get( idx - 1);
        Node prev2 = content.get( idx - 2);
        if ( prev instanceof Text ws && prev2 instanceof Element) {
            String text = ws.getText();
            return text != null && text.trim().isEmpty() && countNewlines( text) >= 2;
        }
        return false;
    }
    ...
    /** True when any direct text child of {@code element} contains a line break. */
    private static boolean containsNewlineText( Element element) {
    ...
    protected void writeDocType( DocumentType docType) throws IOException {
        ...
        if (docType != null) {
            ...
            // End the doctype line only; the root element adds the single blank-line gap.
        ...
        for ( int i = 0; i < size; i++ ) {
			...
			if ( node instanceof Element) {
				...
				if ( text != null && text.length() > 0) {
					// Non-whitespace is obviously text. Whitespace WITHOUT a line break is a
					// significant inline space the author typed between elements (e.g.
					// "<ph>a</ph> <ph>b</ph>"), not pretty-print indentation — treat the
					// element as mixed so that space is preserved, not dropped.
...
//			System.out.println( "prefix = "+prefix+" value = "+value);
		...
		// Also treat configured elements (e.g. DITA codeblock/pre/lines/screen) as
		// whitespace-significant, so their verbatim content is never reflowed.
		if ( !preserveSpace && preserveSpaceElements.contains( element.getName())) {
			preserveSpace = true;
		}

		// Preserve a single blank line between block siblings (collapsing runs of
		// blanks to one), the usual code-formatter convention (Prettier/gofmt).
		if ( preserveBlankLines && precededByBlankLine( element)) {
			writePrintln();
		}
        ...
        if ( size <= 0 ) {
            ...
            if ( !hasElement && preserveTextLineBreaks && containsNewlineText( element)) {
                // Text-only element whose author put line breaks in the prose (semantic
                // line breaks / one-sentence-per-line). Preserve them verbatim instead of
                // collapsing to one line — the same path as xml:space, so the breaks
                // survive every format/save and diffs stay minimal.
                writePreservedElementContent( element);
            } else if ( !hasElement && !preserveSpace) { // text only
