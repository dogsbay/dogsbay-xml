...
public class XDiff
{
	...
	// Set the optimal flag by default
	private static boolean		_oFlag = true, _gFlag = false;
	...
	private com.dogsbay.dogsbayaieditor.properties.ConfigurationProperties properties = null;

	public final static String NO_DIFF = "NO_DIFF";
	...
	/**
	  ...
	  */
	public XDiff(String input1, String input2)
	{
		this.input1 = URLUtilities.encodeURL(input1);
		this.input2 = URLUtilities.encodeURL(input2);
		this.properties = null;
	}

	/**
	  * Constructor with grammar type support
	  * @param	input1		input file #1
	  * @param	input2		input file #2
	  * @param	properties	configuration properties for grammar type resolution
	  */
	public XDiff(String input1, String input2, com.dogsbay.dogsbayaieditor.properties.ConfigurationProperties properties)
	{
		this.input1 = URLUtilities.encodeURL(input1);
		this.input2 = URLUtilities.encodeURL(input2);
	...
	{
		// Parse input files
		XParser	parser = new XParser(properties);
		_xtree1 = parser.parse(input1);

		XParser parser2 = new XParser(properties);
		...
		else
		{
			...
			if (_xtree1.getTag2(root1).compareTo(_xtree2.getTag2(root2)) != 0)
			{			
	...
	private void xdiff(int pid1, int pid2, boolean matchFlag)
	{
		...
		else if ((count1 == 1) && (count2 == 1))
		{
			...
			if (isE1 && isE2)
			{
				String	tag1 = _xtree1.getTag2(node1);
				String	tag2 = _xtree2.getTag2(node2);
		...
		else
		{
			...
			while ((muc1 < ucount1) && (muc2 < ucount2))
			{
				...
				String	startTag = _xtree1.getTag2(elements1[start]);
				int	uele1 = 0, uele2 = 0;
				muc1++;
				...
				{
					if (!matched1[i] && startTag.equals(_xtree1.getTag2(elements1[i])))
					{
						matched1[i] = true;
						muc1++;
				...
				{
					if (!matched2[i] && startTag.equals(_xtree2.getTag2(elements2[i])))
					{
						matched2[i] = true;
						muc2++;
	...
	private void diffAttributes(int attrCount1, int attrCount2)
	{
		if ((attrCount1 == 1) && (attrCount2 == 1))
		{
			...
			String	tag1 = _xtree1.getTag2(_attrList1[0]);
			String	tag2 = _xtree2.getTag2(_attrList2[0]);
			if (tag1.compareTo(tag2) == 0)
			{
				_matchp[0] = XTree.CHANGE;
		...
		for (int i = 0; i < attrCount2; i++)
		{
			...
			_attrTag[i] = _xtree2.getTag2(_attrList2[i]);
			_attrMatch[i] = false;
		}
		...
		for (int i = 0; i < attrCount1; i++)
		{
			...
			String	tag1 = _xtree1.getTag2(attr1);
			     ...
			     int threshold)
	{
		...
		if (isE1 && isE2)
		{
			if (_xtree1.getTag2(eid1).compareTo(_xtree2.getTag2(eid2)) != 0)
...
/*
	...
	private int _diffAttributes(int attrCount1, int attrCount2)
	{
		...
		for (int i = 0; i < attrCount1; i++)
		{
			...
			String	tag1 = _xtree1.getTag2(_attrList1[i]);
			boolean	found = false;
	  ...
	  for (int k = 0; k < nodeCount; k++)
	  {
		...
		for (int j = 0; j < nodeCount; j++)
		{
		  if ((j != k) && (_leastCostMatrix[k][j] != XTree.NO_CONNECTION))
		  {
			...
			// Locate the circuit.
			//circuit.addElement(Integer.valueOf(i));
			_circuit[0] = i;
			_circuit[1] = 2;

			//circuit.addElement(Integer.valueOf(pathMatrix[i][i]));
	...
	/**
	  ...
	  * @param	input the first/old xml document as a uri
	  */
	private String writeDiff(String input) throws Exception
	{
		...
		try
		{
			//File file = new File(new URI(input));
			//FileInputStream is = new FileInputStream(file);
			
			InputStream is = URLUtilities.open( URLUtilities.toURL(input));
			...
			//OutputStreamWriter out = new OutputStreamWriter(System.out, _encoding);
			out = new OutputStreamWriter(bos, _encoding);
				     ...
				     int node) throws IOException
	{
		if (_xtree1.isElement(node))
		{
			...
			while (attr > 0)
			{
				...
				String attrQuote = getQuote(value);
				     ...
				     int node) throws IOException
	{
		if (_xtree2.isElement(node))
		{
			...
			//System.out.println("** writeInsertNode:"+tag);
			out.write("<" + tag);
				    ...
				    int node) throws IOException
	{
		if (xtree.isElement(node))
		{
			...
			//System.out.println("** writeMatchNode:"+tag);
			if (_needNewLine)
				out.write("\n");
	...
	private String getQuote(String value)
	{
		if (value.indexOf("\"") != -1)
			return "'";
		else
				   ...
				   int node2) throws IOException
	{
		if (_xtree1.isElement(node1))
		{
			...
			//System.out.println("** writeDiffNode:"+tag);
			if (_needNewLine)
				out.write("\n");
			out.write("<" + tag);
			...
			while (attr1 > 0)
			{
				...
				else
				{
					...
					String attrQuote2 = getQuote(value2);
					out.write(" " + atag +
							"="+attrQuote2+
							//"=\"" +
						  value2 +
			...
			while (child1 > 0 || child2 > 0)
			{
				...
				{
					//System.out.println("child1 > 0:"+child1+" child2 > 0:"+child2);
				...
				{
					//System.out.println("** child1 > 0 **:"+child1);
				...
				{
					//System.out.println("** child2 > 0 **:"+child2);
...
//				
//				_xtree2.getMatching(child2, _matchp);
//				String	tagName = _xtree2.getTag(child2);
//				System.out.println("** child2 tagname:"+tagName);
//				
//				if (_matchp[0] == XTree.INSERT)
//					writeInsertNode(out, child2);
//
...
//			}
	...
	public static void main(String args[]) throws Exception
	{
		...
		{
			System.err.println("XDiff::main - "+_usage);
			return;
		}
		...
		String result = mydiff.getDiff();
		System.out.println("diffgram result:"+result);
	}
