...
class XParser extends DefaultHandler implements LexicalHandler
{
	...
	private com.dogsbay.dogsbayaieditor.properties.ConfigurationProperties _properties;
	...
	/**
	  ...
	  */
	public XParser(com.dogsbay.dogsbayaieditor.properties.ConfigurationProperties properties) throws Exception
	{
		...
		try
		{
			...
			// Set up entity resolver with grammar type support
			_parser.setEntityResolver(new GrammarTypeEntityResolver(_properties));
...
//		}
	...
	/**
	 * Custom EntityResolver that uses catalog resolution and grammar types.
	 * This allows XML Diff to work with configured grammar types when catalogs aren't set up.
	 */
	private static class GrammarTypeEntityResolver implements EntityResolver
	{
		...
		{
			try {
				// Use XMLUtilities to resolve via catalogs and grammar types
				String resolved = XMLUtilities.resolveWithGrammarTypes(publicId, systemId, properties);

				if (resolved != null) {
					java.net.URL url = com.dogsbay.dogsbayaieditor.URLUtilities.toURL(resolved);
					InputSource source = new InputSource(resolved);
					source.setByteStream(url.openStream());
					return source;
				}
			} catch (Exception e) {
				// If resolution fails, return empty source to avoid parsing errors
				return new InputSource(new java.io.StringReader(""));
			}

			// Fallback: return original systemId
			return null;
		}
	}
}
