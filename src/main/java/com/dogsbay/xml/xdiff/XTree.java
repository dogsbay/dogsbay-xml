...
class XTree {
	...
	private String		_valueTagURI[][];
	private Hashtable	_tagNames, _cdataTable;
	
	private long rootHash = 0;
	...
	private void _initialize()
	{
		...
		_valueTagURI = new String[_topCap][];

		_value[0]	= new String[_botCap];
		_valueTagURI[0] = new String[_botCap];
	...
	/**
	  ...
	  */
	public int addElement(int pid, int lsid, String tagName, String tagURI)
	{
		_elementIndex++;
		...
		{
			_tagIndex++;
			tagID = Integer.valueOf(_tagIndex);
			_value[0][_tagIndex] = tagName;
			_valueTagURI[0][_tagIndex] = tagURI;
	...
	public int addText(int eid, int lsid, String text, long value)
	{
		...
		//_valueTagURI[vtopid][vbotid] = text;
	...
	public int addAttribute(int eid, int lsid, String name, String value,
				long valuehash, long attrhash, String attrNameURI)
	{
		// attribute name first.
		int	aid = addElement(eid, lsid, name,attrNameURI);
	...
	{
		Integer	key = Integer.valueOf(eid);
		Object	value = _cdataTable.get(key);
		if (value == null)
		{
			Vector	list = new Vector(2);
			list.addElement(Integer.valueOf(position));
	...
	{
		return (Vector)_cdataTable.get(Integer.valueOf(eid));
	}
	...
	public String getTag(int eid)
	{
		...
		//System.out.println("** return tag:"+_value[0][index]);
		return	_value[0][index];
	}
	
	/**
	  * Get the uri and tag combination of an element node
	  * @param	eid	element id
	  */
	public String getTag2(int eid)
	{
		int	index = _valueIndex[eid/_botCap][eid%_botCap];
		//System.out.println("** return tag2:"+_valueTagURI[0][index]);
		return	_valueTagURI[0][index];
	}
	...
	public void setRootHash(long rootHash)
	{
