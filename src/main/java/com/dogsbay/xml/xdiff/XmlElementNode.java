...
public class XmlElementNode extends DefaultMutableTreeNode {
	...
	private ImageIcon loadedIcon = DogsBayImageLoader.get().getImage( "com/dogsbay/dogsbayaieditor/icons/XDiff5.gif");
...
//		}
	...
	protected Line parseContent( Vector lines, Line current, String text) {
		...
		if (text != null && (current.length()+1 >= MAX_LINE_LENGTH) && (text.length() > 0)) {
			current = new Line();
			lines.add( current);
			current.addStyledString( TAB);
		}
