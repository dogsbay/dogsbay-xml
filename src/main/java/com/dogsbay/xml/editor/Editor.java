...
import com.dogsbay.xml.CommonEntities;
...
/**
 * The editor for a DogsBay XML document.
 *
 * @version	$Revision: 1.73 $, $Date: 2005/08/09 14:32:31 $
 * @author Dogsbay
 */
public class Editor extends ViewPanel implements CaretListener,
        UndoableEditListener, DocumentListener, DogsBayDocumentListener {

    private static final boolean DEBUG = false;
    private static final ImageIcon ICON = DogsBayImageLoader.get().getImage(
            "com/dogsbay/xml/editor/icons/EditorIcon.gif");
     ...
     *
     * @param parent the DogsBayAIEditor component.
     * @param props the properties for the editor.
     */
    public Editor(DogsBayAIEditor _parent, ConfigurationProperties props,
            DogsBayView _view, ErrorList errors) {
        ...
        // Initialize and add FindBar (VSCode-style find/replace bar)
    ...
    private EditorPanel createEditorPanel(boolean init) {
            ...
            public void focusGained(FocusEvent e) {
                ...
                if (editor != null) {
                    ...
                    if (parent != null) {
                        ...
                        // Update file explorer to highlight the current file
                        if (parent.getFileExplorer() != null && document != null) {
                            java.net.URL docURL = document.getURL();
                            if (docURL != null && "file".equals(docURL.getProtocol())) {
                                try {
                                    java.io.File file = new java.io.File(docURL.toURI());
                                    parent.getFileExplorer().selectFile(file);
                                } catch (Exception ex) {
        ...
        editor.getActionMap().put("dogsbay-indent", tabAction);
        editor.getActionMap().put("dogsbay-unindent", unindentAction);
        ...
        editor.getInputMap().put(
                KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, Toolkit
                        ...
                        + InputEvent.SHIFT_DOWN_MASK, false), "toggle-full-fold");
     ...
     */
    /**
     * Put the editor's keys where the reader asked for them.
     *
     * <p>A table, where this was a chain of eighteen
     * {@code else if (actionName.equals(...))} blocks each doing the same
     * three things — and each of which had to be written by hand before an
     * editor command could carry a key at all.
     */
    public void setKeyMappings(DogsBayAIEditor parent, String configName) {
        var bindings = props.getKeyBindings();
        editorKeySetters().forEach((id, setter) ->
                setter.accept(bindings.strokeFor(id).orElse(null)));
    }

    /** Which editor command each binding drives. */
    private java.util.Map<String, java.util.function.Consumer<KeyStroke>> editorKeySetters() {
        var setters = new java.util.LinkedHashMap<String, java.util.function.Consumer<KeyStroke>>();
        setters.put(KeyPreferences.SELECT_ALL_ACTION, this::setSelectAllActionKey);
        setters.put(KeyPreferences.TAB_ACTION, this::setTabActionKey);
        setters.put(KeyPreferences.UNINDENT_ACTION, this::setUnindentActionKey);
        setters.put(KeyPreferences.BEGIN_ACTION, this::setBeginActionKey);
        setters.put(KeyPreferences.BEGIN_LINE_ACTION, this::setBeginLineActionKey);
        setters.put(KeyPreferences.END_ACTION, this::setEndActionKey);
        setters.put(KeyPreferences.END_LINE_ACTION, this::setEndLineActionKey);
        setters.put(KeyPreferences.UP_ACTION, this::setUpActionKey);
        setters.put(KeyPreferences.DOWN_ACTION, this::setDownActionKey);
        setters.put(KeyPreferences.LEFT_ACTION, this::setLeftActionKey);
        setters.put(KeyPreferences.RIGHT_ACTION, this::setRightActionKey);
        setters.put(KeyPreferences.NEXT_WORD_ACTION, this::setNextWordActionKey);
        setters.put(KeyPreferences.PREVIOUS_WORD_ACTION, this::setPreviousWordActionKey);
        setters.put(KeyPreferences.PAGE_UP_ACTION, this::setPageUpActionKey);
        setters.put(KeyPreferences.PAGE_DOWN_ACTION, this::setPageDownActionKey);
        setters.put(KeyPreferences.DELETE_NEXT_CHAR_ACTION, this::setDeleteNextCharActionKey);
        setters.put(KeyPreferences.DELETE_PREV_CHAR_ACTION, this::setDeletePrevCharActionKey);
        setters.put(KeyPreferences.GOTO_START_TAG_ACTION, this::setGoToStartTagActionKey);
        setters.put(KeyPreferences.GOTO_END_TAG_ACTION, this::setGoToEndTagActionKey);
        setters.put(KeyPreferences.GOTO_NEXT_ATTRIBUTE_VALUE_ACTION, this::setGoToNextAttributeValueActionKey);
        setters.put(KeyPreferences.GOTO_PREVIOUS_ATTRIBUTE_VALUE_ACTION, this::setGoToPreviousAttributeValueActionKey);
        return setters;
    }
    ...
    private void setTabActionKey(KeyStroke stroke) {
        ...
        removePreviousMapping("dogsbay-indent");
        bottomEditor.getEditor().getInputMap(JComponent.WHEN_FOCUSED).put(
                stroke, "dogsbay-indent");
        topEditor.getEditor().getInputMap(JComponent.WHEN_FOCUSED).put(stroke,
                "dogsbay-indent");
    }
    ...
    private void setUnindentActionKey(KeyStroke stroke) {
        ...
        removePreviousMapping("dogsbay-unindent");
        bottomEditor.getEditor().getInputMap(JComponent.WHEN_FOCUSED).put(
                stroke, "dogsbay-unindent");
        topEditor.getEditor().getInputMap(JComponent.WHEN_FOCUSED).put(stroke,
                "dogsbay-unindent");
    }
    ...
    public void updateFragmentKeys() {
        ...
        if (fragments != null) {
            for (int i = 0; i < fragments.size(); i++) {
                ...
                String name = "dogsbay.editor.fragment." + i;
    ...
    private void removeFragmentKeys() {
        ...
        for (int i = 0; i < keys.length; i++) {
            ...
            if (key instanceof String
                    && ((String) key).startsWith("dogsbay.editor.fragment.")) {
    ...
    public void setDocument(DogsBayDocument document) {
        ...
        if (document != null) {
            ...
            // Use the document's format to select the appropriate editor kit
            DocumentFormat format = document.getDocumentFormat();
            String contentType = (format != null) ? format.getContentType() : "text/txt";

            bottomEditor.getEditor().setContentType(contentType);
            bottomEditor.getEditor().refreshEditorKit();
            if (topEditor.isInitialised()) {
                topEditor.getEditor().setContentType(contentType);
                topEditor.getEditor().refreshEditorKit();
            }
        ...
        if (document != null) {
            DocumentFormat fmt = document.getDocumentFormat();
            if (fmt != null && !fmt.isXmlBased()) {
                bottomEditor.updatePreferences();
                if (topEditor.isInitialised()) {
                    topEditor.updatePreferences();
                }
    ...
    public void selectElement(XElement element, boolean endTag, int y) {
        // If we're already syncing, don't select (prevents tree view from overriding user clicks)
        if (syncingSelection) {
            return;
        }

        // Prevent circular sync updates
    ...
    public void gotoLine(int line) {
        // Prevent auto-sync during goto to avoid confusing tree highlights
        syncingSelection = true;
        try {
    ...
    private void showPopupMenu(MouseEvent e) {
        ...
        // Plugin-contributed, click-context-aware items (e.g. key navigation).
        // Contributors decide from the click offset whether they have anything
        // to offer; an empty contribution leaves the popup unchanged.
        int clickOffset = getEditor().viewToModel2D(new Point(e.getX(), e.getY()));
        java.util.List<javax.swing.JMenuItem> contributed =
                EditorPopupContributors.collect(getEditor(), clickOffset);
        if (!contributed.isEmpty()) {
            for (javax.swing.JMenuItem item : contributed) {
                popup.add(item);
            }
            popup.addSeparator();
        }
        ...
        if (selectedText != null) {
            String fullSelection = selectedText;
            if (selectedText.length() > 20) {
                selectedText = selectedText.substring(0, 17);
                selectedText = selectedText + "...";
            }

            // Clipboard — the most common actions on a selection, at the top.
            popup.add(parent.getCutAction());
            popup.add(parent.getCopyAction());
            popup.add(parent.getPasteAction());
            popup.addSeparator();

            // Find / Open. Only offer "Open" when the selection is a single
            // reference token (an href/URL), not arbitrary prose: getCurrentURL()
            // resolves the whole selection against the document base, so a
            // multi-line paragraph would otherwise produce a bogus file: URL whose
            // full text lands in the label and blows the popup out to window width.
            popup.add(new WrapperAction(parent.getFindAction(), "Find \""
                    + selectedText + "\""));
            URL url = isOpenableReference(fullSelection) ? getCurrentURL() : null;
            if (url != null) {
                popup.add(new WrapperAction(parent.getOpenCurrentURLAction(),
                        "Open \"" + ellipsizeLabel(URLUtilities.toDisplay(url), 60) + "\""));
            }
            popup.addSeparator();

            // XML-specific transforms, grouped under one submenu so the top-level
            // menu stays short and contextual. (Document lifecycle actions —
            // Save/Save As/Close — are deliberately omitted: they aren't operations
            // on the selection and live on the File menu, toolbar and tab.)
            javax.swing.JMenu xml = new javax.swing.JMenu("XML");
            Action toggleEmptyElementAction = parent.getToggleEmptyElementAction();
            if (toggleEmptyElementAction.isEnabled()) {
                xml.add(new WrapperAction(toggleEmptyElementAction,
                        "Expand Empty Element"));
            }
            Action tagAction = parent.getTagAction();
            if (tagAction.isEnabled()) {
                xml.add(new WrapperAction(tagAction, "Tag Selection"));
            }
            Action commentAction = parent.getCommentAction();
            if (commentAction.isEnabled()) {
                xml.add(new WrapperAction(commentAction, commentAction
                        .getValue(Action.NAME) + " Selection"));
            }
            Action cdataAction = parent.getCDATAAction();
            if (cdataAction.isEnabled()) {
                xml.add(new WrapperAction(cdataAction, cdataAction
                        .getValue(Action.NAME) + " Selection"));
            }
            if (xml.getMenuComponentCount() > 0) {
                xml.addSeparator();
            }
            xml.add(parent.getSubstituteCharactersAction());
            xml.add(parent.getSubstituteEntitiesAction());
            xml.add(parent.getStripTagsAction());
            xml.addSeparator();
            xml.add(parent.getParseAction());
            xml.add(parent.getValidateAction());
            popup.add(xml);
        }
        else {
            int pos = getEditor().viewToModel2D(new Point(e.getX(), e.getY()));
            ...
            popup.add(parent.getPasteAction());
            popup.addSeparator();
            // Document-wide XML checks, grouped to match the selection menu.
            // (Save/Save As/Close omitted — available on File menu, toolbar, tab.)
            javax.swing.JMenu xml = new javax.swing.JMenu("XML");
            xml.add(parent.getParseAction());
            xml.add(parent.getValidateAction());
            popup.add(xml);
    ...
    /**
     * Gets the find bar component.
     * @return the FindBar instance
     */
    ...
    /**
     * True when a selection is a single openable reference (an href/URL token)
     * rather than prose or markup. Used to gate the "Open" context-menu item so
     * a multi-line selection can't be resolved into a giant bogus URL label.
     */
    static boolean isOpenableReference(String selection) {
        if (selection == null) {
            return false;
        }
        String s = selection.trim();
        if (s.isEmpty() || s.length() > 1024) {
            return false;
        }
        // Reject only what clearly isn't a single reference: a multi-line selection
        // (a paragraph, the original giant-menu bug) or one containing markup. A
        // single-line href/path — even one with spaces, e.g. "my topic.dita" — is
        // allowed; the label itself is length-capped so the menu can't blow out.
    ...
    /** Caps a menu-item label, appending an ellipsis when it is truncated. */
    static String ellipsizeLabel(String s, int max) {
        if (s == null) {
            return "";
        }
    ...
    /*	public void format() throws SAXParseException, IOException {
     ...
     licenseManager.isValid( com.dogsbay.license.KeyGenerator.generate(7), "DogsBay XML");
     } catch (Exception x) {
     System.exit(0);
     return;
     }
    ...
    private Vector getCurrentAttributeValues() {
        ...
        if (getEditor() != null) {
            ...
            if (tag != null) {
                ...
                Vector values = getAttributeValues(attribute);
                return mergeContributedAttributeValues(values, tag, attribute);
            }
        }
        return null;
    }
    ...
    /**
     * Merge plugin-contributed attribute values (e.g. a DITA subjectScheme's
     * controlled values) into the grammar-provided ones, de-duplicated and marked
     * as {@link SchemeAttributeValue} so the popup can render them distinctly. Any
     * failure leaves the grammar values untouched.
     */
    private Vector mergeContributedAttributeValues(Vector values, Tag tag, String qname) {
        if (qname == null) {
            return values;
        }
        ...
        try {
            ...
            String publicId = (document != null) ? document.getPublicID() : null;
            extra = AttributeValueContributors.collect(
                    (tag != null) ? tag.getName() : null, stripPrefix(qname), file, publicId);
        } catch (Throwable t) {
            return values;
        }
        ...
        Vector results = (values != null) ? values : new Vector();
        for (String v : extra) {
            if (v != null && !v.isEmpty() && !attributeValuesContain(results, v)) {
                results.addElement(new SchemeAttributeValue(v));
            }
        }
        return results;
    }

    private static boolean attributeValuesContain(Vector values, String v) {
        for (int i = 0; i < values.size(); i++) {
            Object o = values.elementAt(i);
            if (o instanceof com.dogsbay.schema.AttributeValue
                    && v.equals(((com.dogsbay.schema.AttributeValue) o).getValue())) {
                return true;
            }
            if (v.equals(String.valueOf(o))) {
                return true;
            }
        }
        return false;
    ...
    public void updateHelper() {
        ...
        if (document != null) {
            ...
            if (node != null) {
                if (node instanceof XElement) {
                    // Sync with Navigator (prevent circular updates)
                    if (!syncingSelection) {
                        syncingSelection = true;
                        try {
                    ...
                    // Auto-sync with sidebar Viewer if enabled
                    if (!syncingSelection && parent.isAutoSynchroniseSelection() && parent.getSidebarViewer() != null) {
                        syncingSelection = true;
                        try {
                            parent.getSidebarViewer().setSelectedElement((XElement) node, false, -1);
                        } finally {
                            syncingSelection = false;
                        }
                    }
                    ...
                    if (!syncingSelection && parent.isAutoSynchroniseSelection() && parent.getSidebarViewer() != null) {
                        syncingSelection = true;
                        try {
                            parent.getSidebarViewer().setSelectedElement((XElement) ((XAttribute) node).getParent(), false, -1);
                        } finally {
                            syncingSelection = false;
                        }
                    }
    ...
    class FragmentAction extends AbstractAction {
        ...
        public FragmentAction(FragmentProperties fragment) {
            ...
            if (keySequence != null) {
                ...
                putValue(ACCELERATOR_KEY, com.dogsbay.dogsbayaieditor.properties.KeyBinding.stroke(
                        mask == null ? null : mask.toUpperCase(java.util.Locale.ROOT).replace("+", "_"),
                        value));
            }
        ...
        private void setIcon() {
            ...
            if (icon == null) {
                icon = DogsBayImageLoader
                        .get()
                        .getImage(
                                "com/dogsbay/dogsbayaieditor/icons/DefaultFragmentIcon.gif");
            }
