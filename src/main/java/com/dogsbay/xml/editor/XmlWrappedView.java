...
import com.dogsbay.xml.editor.XmlView.Prefix;
 ...
 * @author Dogsbay
 */
public class XmlWrappedView extends FoldingWrappedPlainView {
	 ...
	 */
	public XmlWrappedView( XmlContext context, Element elem, ErrorList errors) {
	    super( elem, true);
	...
	/**
	 * Construct a colorized view of xml text for the element 
	 * using the supplied scanner object.
	 *
	 * @param context the styles used to colorize the view.
	 * @param elem the element to create the view for.
	 */
	public XmlWrappedView( XmlContext context, Element elem, XmlScanner scanner) {
	    super( elem, true);
	 ...
	 */
	public int calculateLineCount( Element e) {
	    int nlines = 0;
	    int p1 = e.getEndOffset();

	    for (int p0 = e.getStartOffset(); p0 < p1; ) {
	        nlines += 1;
