...
/**
 * Wrapped view for rendering Markdown with syntax highlighting.
 */
public class MarkdownWrappedView extends FoldingWrappedPlainView {
	private boolean highlight = false;
	private MarkdownScanner lexer;
	private boolean lexerValid;
	private XmlContext context = null;

	public MarkdownWrappedView(XmlContext context, Element elem) {
		super(elem, true);
	...
	protected int drawUnselectedText(Graphics g, int x, int y, int start, int end) throws BadLocationException {
		...
		// Always use the plain font so that character widths match the view's
		// modelToView/viewToModel calculations. Only change color per token.
		java.awt.Font plainFont = context.getFont(context.getStyle(Constants.MD_TEXT));
		g.setFont(plainFont);
