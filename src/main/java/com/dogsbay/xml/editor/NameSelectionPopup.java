...
public class NameSelectionPopup extends JPopupMenu {
	...
	private static final ImageIcon TEXT_ICON = DogsBayImageLoader.get().getImage( "com/dogsbay/dogsbayaieditor/icons/ExclamationIcon.gif");
	private static final ImageIcon ENTITY_ICON = DogsBayImageLoader.get().getImage( "com/dogsbay/dogsbayaieditor/icons/EntityIcon.gif");
	private static final ImageIcon NAMESPACE_ICON = DogsBayImageLoader.get().getImage( "com/dogsbay/dogsbayaieditor/icons/NamespaceIcon.gif");
	...
	private static final ImageIcon ATTRIBUTE_REQUIRED_ICON = DogsBayImageLoader.get().getImage( "com/dogsbay/xml/designer/icons/RequiredAttributeIcon.gif");
	...
	class NameListCellRenderer extends JPanel implements ListCellRenderer {
		...
		public Component getListCellRendererComponent( JList list, Object value, int index, boolean selected, boolean focus) {
			...
			if ( value instanceof AttributeInformation) {
				...
				setEntityValue( -1);
			} else if ( value instanceof SchemeAttributeValue){
				// Controlled value from a DITA subjectScheme — render distinctly
				// (a subtle accent colour) so it reads apart from grammar values.
				setIcon( null);
				setForeground( selected ? list.getSelectionForeground()
						: new java.awt.Color( 0x0B, 0x66, 0xC2));
				setPrefix( null);
				setText( value.toString());
				name.setFont( name.getFont().deriveFont( java.awt.Font.PLAIN));
				setValue( null);
				setEntityValue( -1);
