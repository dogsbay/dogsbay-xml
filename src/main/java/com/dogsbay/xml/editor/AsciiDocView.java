...
/**
 * View for rendering AsciiDoc with syntax highlighting.
 * Follows the same pattern as MarkdownView.
 */
public class AsciiDocView extends FoldingPlainView {
    private boolean highlight = false;
    private AsciiDocScanner lexer;
    private boolean lexerValid;
    private XmlContext context = null;

    public AsciiDocView(XmlContext context, Element elem) {
        super(elem);
        this.context = context;

        XmlDocument doc = (XmlDocument) getDocument();
        try {
            lexer = new AsciiDocScanner(doc);
        } catch (Exception e) {
            lexer = null;
        }
        lexerValid = false;
    ...
    private Style resolveStyle(Style style) {
        if (style == null) {
            Style defaultStyle = context.getStyle(Constants.AD_TEXT);
            return (defaultStyle != null) ? defaultStyle : context.getStyle(0);
        }
        return style;
    }
