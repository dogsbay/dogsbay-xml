...
public class CommonEntities {
	...
	public static String getEntityName( char character) {
		...
		return (String)entitiesByCharacter.get( Character.valueOf( character));
	}
				...
				for ( int j = 0; j < entities.length; j++) {
					...
					for ( int k = 0; k < orderedCharacters.size(); k++) {
						...
						if ( character < orderedCharacter) {
							orderedCharacters.insertElementAt( Character.valueOf( character), k);
							entitiesByCharacter.put( Character.valueOf( character), entityName);
							append = false;
							break;
					...
					if ( append) {
						entitiesByCharacter.put( Character.valueOf( character), entityName);
						orderedCharacters.addElement( Character.valueOf( character));
					}
				}
			}
		}
