...
public class RNGDocument implements com.dogsbay.schema.SchemaDocument {
	...
	public RNGDocument( URL location, boolean compact) throws IOException, SAXException {
		...
		try {
			// Trang 20241231 takes the resolver explicitly rather than building one
			// internally. BasicResolver is the library's own default, and is what
			// Trang's own driver uses when no catalog has been configured.
			SchemaCollection schemaCollection = inputFormat.load(
					location.toString(), new String[0], ".rng", handler, BasicResolver.getInstance());
