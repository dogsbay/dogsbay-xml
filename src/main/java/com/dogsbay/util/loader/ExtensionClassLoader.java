...
package com.dogsbay.util.loader;
...
public class ExtensionClassLoader extends URLClassLoader {
    ...
    public void addLibrary( File library) {
        ...
        if ( library.exists()) {
	        try  {
	            URL url = DogsBayURLUtilities.getURLFromFile(library.getAbsoluteFile());
    ...
    public void addExtension( File extension) {
        ...
        if ( extension.exists()) {
            try  {
                URL url = DogsBayURLUtilities.getURLFromFile(extension.getAbsoluteFile());
            ...
            for ( int i = 0; i < pluginsDirFiles.length; i++) {
            	if(pluginsDirFiles[i].isDirectory() == true) {
	                ...
	                if ( pluginsDirFiles[i].getAbsolutePath().toLowerCase().endsWith(".jar")) {
	                    try  {
	                        URL url = DogsBayURLUtilities.getURLFromFile(pluginsDirFiles[i].getAbsoluteFile());
	                    	if (VERBOSE) System.out.println("Adding plugin: " + url.getPath());
        ...
        if ( pluginsLibDir.exists() && pluginsLibDir.isDirectory()) {
            ...
            for ( int i = 0; i < jars.length; i++) {
                if ( jars[i].getAbsolutePath().toLowerCase().endsWith(".jar")) {
                    try  {
                        URL url = DogsBayURLUtilities.getURLFromFile(jars[i].getAbsoluteFile());
